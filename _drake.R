...
transmissionDates <- tribble(
  ...
  "2020-04-01", round(0.51*708), round(0.3*708), round(0.18 * 708), round(0.01*708),
  "2020-04-02", round(0.51*797), round(0.31*797), round(0.17*797), round(0.01*797)
  ) %>% mutate(Date=as.Date(Date))
...
plan <- drake_plan(
    ...
    confirmedCases = readxl::read_excel(file_in(here::here("data/moh/covid-19_case_list_2_april_2020.xlsx")), skip=3) %>%
      mutate(Overseas=`International travel`=="Yes", Status="Confirmed"),

    probableCases = readxl::read_excel(file_in(here::here("data/moh/covid-19_case_list_2_april_2020.xlsx")), skip=3, sheet=2) %>%
